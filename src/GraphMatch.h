...
public:
    ...
    /** 
     * Get the number of edges in GraphMatch
    */
    const int numEdges() const {return _edges.size(); }
    /**
    ...
    /** 
     * Creat hedge to gedge map
     */
    void createHEdge2gEdgeMap();
    /** 
     * Gets the index in the original graph that's associate with the matched edge in the query graph.
     */
    int getGraphEdge(int hEdge);
    /**
...
private:
    ...
    std::unordered_map<int,int> _hEdge2gEdgeMap;
};
